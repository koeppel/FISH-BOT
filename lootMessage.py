...
def removeLootMessage(lootMessage:LootMessage|None):
    lootMessages = getLootMessages()
    if lootMessage and lootMessage in lootMessages:
...
async def createLootMessage(channel:TextChannel, item:JSONItem, author:User|Member):
    ...
    print(f"Created message for [{item['name']}] in channel '{channel.name}'")
...
async def deleteLootMessage(channel:TextChannel, messageID:int):
    ...
    if message:
        ...
        if lootMessage:
            print(f"Deleting message for [{lootMessage['item']['name']}] in channel '{channel.name}'")
            removeLootMessage(lootMessage)
